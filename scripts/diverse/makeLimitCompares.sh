...
do
    ...
    do
        ...
        if [ $CHN == "em" ]; then LABEL="e^{}#mu"; continue; fi
        ...
        if [ $CHN == "htt_em_nobtag_13TeV" ]; then LABEL="e^{}#mu no b-tag"; continue; fi
        if [ $CHN == "htt_em_btag_13TeV" ]; then LABEL="e^{}#mu b-tag"; continue; fi
        if [ $CHN == "htt_em_8_13TeV" ]; then LABEL="e^{}#mu no b-tag low D^{}_{#zeta}"; continue; fi
        if [ $CHN == "htt_em_9_13TeV" ]; then LABEL="e^{}#mu b-tag low D^{}_{#zeta}"; continue; fi
        if [ $CHN == "htt_em_10_13TeV" ]; then LABEL="e^{}#mu no b-tag medium D^{}_{#zeta}"; continue; fi
        if [ $CHN == "htt_em_11_13TeV" ]; then LABEL="e^{}#mu b-tag medium D^{}_{#zeta}"; continue; fi
        if [ $CHN == "htt_em_12_13TeV" ]; then LABEL="e^{}#mu no b-tag high D^{}_{#zeta}"; continue; fi
        if [ $CHN == "htt_em_13_13TeV" ]; then LABEL="e^{}#mu b-tag high D^{}_{#zeta}"; continue; fi
        ...
        python scripts/plotMSSMLimits.py ${FILE} ${FILE//freezing_/freezing_classic_}:exp0:LineStyle=7,LineWidth=2,LineColor=ROOT.kBlue,MarkerColor=ROOT.kBlue,MarkerStyle=8,Title='"'"Classic Method"'"' --output mssm_freezing_${PROC}H_comp_FF_classic_${CHN} --show exp --process "${PROC}#phi" --title-left "${LABEL}" --logy --logx --cms-sub "Preliminary" --legend-pos "0.2,0.15,3,0.015" --ratio-to ${FILE}:exp0 --title-right "35.9 fb^{-1} (13 TeV)"
        #python scripts/plotMSSMLimits.py ${FILE}:exp0:LineStyle=7,LineWidth=2,LineColor=ROOT.kBlue,MarkerColor=ROOT.kBlue,MarkerSize=1.2,MarkerStyle=22,Title='"'"FF Method"'"' ${FILE//freezing_/freezing_classic_}:exp0:LineStyle=7,LineWidth=2,LineColor=ROOT.kRed,MarkerColor=ROOT.kRed,MarkerStyle=21,Title='"'"Classic Method"'"' --output mssm_freezing_${PROC}H_comp_FF_classic_${CHN} --show exp --process "${PROC}#phi" --title-left "${LABEL}" --logy --logx --cms-sub "Preliminary" --legend-pos "0.2,0.15,3,0.015" --ratio-to ${FILE}:exp0 --title-right "35.9 fb^{-1} (13 TeV)"
    done
    ...
    do
        ...
        if [ $CHN == "mt" ]; then LABEL="#mu^{}#tau_{h}"; fi
        if [ $CHN == "cmb" ]; then LABEL="e^{}#tau_{h}^{}+#mu^{}#tau_{h}^{}+#tau_{h}^{}#tau_{h}^{}+e^{}#mu"; fi
        #python scripts/plotMSSMLimits.py mssm_freezing_${PROC}H_${CHN}.json:exp0:LineStyle=7,LineWidth=2,LineColor=ROOT.kBlue,MarkerColor=ROOT.kBlue,MarkerSize=1.2,MarkerStyle=22,Title='"'"HIG-17-020"'"' LimitJsons/mssm_111016_unblinding_${PROC}H_${CHN}.json:exp0:LineStyle=7,LineWidth=2,LineColor=ROOT.kRed,MarkerColor=ROOT.kRed,MarkerStyle=21,Title='"'"HIG-16-037"'"' --output mssm_freezing_${PROC}H_comp_HIG16037_${CHN} --show exp --process "${PROC}#phi" --title-left "${LABEL}" --logy --logx --cms-sub "Preliminary" --legend-pos "0.2,0.15,3,0.015" --ratio-to LimitJsons/mssm_111016_unblinding_${PROC}H_${CHN}.json:exp0
    done
done
